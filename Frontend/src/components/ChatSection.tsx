    ...
    if (socket && senderId && receiverId) {
      ...
      const fetchUser = async ()=>{
        ...
        try {
          ...
          const response= await axios.get(`${import.meta.env.VITE_BASE_URL}/api/users/getfriend/${receiverId}`,{headers: {
      ...
      const fetchMessages = async () => {
        ...
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_BASE_URL}/api/messages/chat/${Math.min(Number(senderId), Number(receiverId))}/${Math.max(
              Number(senderId),
              Number(receiverId)
            )}`,{headers: {
              Authorization: `Bearer ${token}`,
